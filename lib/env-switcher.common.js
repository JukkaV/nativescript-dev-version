...
class EnvSwitcherCommon {
    ...
    changePackageId() {
        ...
        const nsConfigFile = path.join(this.projectData.projectDir, 'nativescript.config.ts');
        const nsConfigBackup = path.join(this.projectData.projectDir, 'nativescript.config.orig.ts');
        ...
        if (fs.existsSync(nsConfigBackup)) {
            this.logger.info('skipping nativescript.config.ts backup...');
        } else {
            if (fs.writeFileSync(packageJSONFile, JSON.stringify(packageJSONBackup, null, 4))) {
                this.logger.info(' backed up package.json file to package.orig.json ');
            }
    ...
    changeAppName() {
        ...
        if (this.platformName === 'android') {
            ...
            manifest.$('activity').attr('android:label', this.currentEnvironment.appName);
